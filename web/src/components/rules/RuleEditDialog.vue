...
import { ruleTypeOptions, type RuleDetail, type RuleFormPayload } from '../../types/rules'
...
watch(
  ...
  (value) => {
    form.id = value?.id
    form.ruleCode = value?.ruleCode || ''
    form.name = value?.name || ''
    form.type = value?.type || 'MID'
  ...
  >
    <el-form label-position="top" class="rule-form">
      ...
      <el-form-item label="规则编码">
        <el-input v-model="form.ruleCode" placeholder="例如 fatigue-high" />
      </el-form-item>
      ...
      <el-form-item label="阈值">
        <el-input-number v-model="form.threshold" :min="0" :max="1" :step="0.01" :precision="2" />
      </el-form-item>
