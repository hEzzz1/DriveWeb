...
import {
  ...
  getPlatformInternalUserAudits,
  getPlatformInternalUserDetail,
  getPlatformInternalUserList,
  getPlatformInternalUserRoleOptions,
  resetPlatformInternalUserPassword,
  updatePlatformInternalUser,
  updatePlatformInternalUserRoles,
  updatePlatformInternalUserStatus,
} from '../api/users'
...
const PLATFORM_ASSIGNABLE_ROLES = [
...
const summaryItems = computed(() => [
  { label: '平台内部账号', value: total.value },
...
async function fetchList(): Promise<void> {
  ...
  try {
    const data = await getPlatformInternalUserList(buildQuery())
    ...
    errorText.value = error instanceof Error ? error.message : '平台内部账号列表加载失败'
  } finally {
    loading.value = false
  }
}
...
async function fetchRoleOptions(): Promise<void> {
  ...
  try {
    const options = await getPlatformInternalUserRoleOptions()
    roleOptions.value = options.filter((item) =>
      PLATFORM_ASSIGNABLE_ROLES.includes(item.roleCode as (typeof PLATFORM_ASSIGNABLE_ROLES)[number]),
    )
  } finally {
...
async function fetchUserAudits(): Promise<void> {
  ...
  try {
    const data = await getPlatformInternalUserAudits(activeDetail.value.id, {
...
async function handleOpenDetail(row: UserSummary): Promise<void> {
  ...
  try {
    activeDetail.value = await getPlatformInternalUserDetail(row.id)
...
async function handleCreateUser(payload: Parameters<typeof createPlatformInternalUser>[0]): Promise<void> {
  createSaving.value = true

  try {
    await createPlatformInternalUser({
      ...payload,
      enterpriseId: null,
    })
    createVisible.value = false
    await fetchList()
    ElMessage.success('平台内部账号已创建')
  } finally {
    createSaving.value = false
  }
}

async function handleEditUser(payload: Parameters<typeof updatePlatformInternalUser>[1]): Promise<void> {
  if (!activeDetail.value) {
    return
  }
  ...
  try {
    const detail = await updatePlatformInternalUser(activeDetail.value.id, {
      ...payload,
      enterpriseId: null,
    })
...
async function handleSaveRoles(roles: UserDetail['roles']): Promise<void> {
  ...
  try {
    const detail = await updatePlatformInternalUserRoles(activeDetail.value.id, { roles })
    activeDetail.value = detail
    syncTableRow(detail)
...
async function handleResetPassword(payload: Parameters<typeof resetPlatformInternalUserPassword>[1]): Promise<void> {
  if (!activeDetail.value) {
    return
  }
  ...
  try {
    const detail = await resetPlatformInternalUserPassword(activeDetail.value.id, payload)
    activeDetail.value = detail
    syncTableRow(detail)
...
async function handleToggleStatus(): Promise<void> {
  ...
  try {
    await ElMessageBox.confirm(`确认${actionText}平台内部账号「${activeDetail.value.username}」吗？`, `${actionText}账号`, {
      type: 'warning',
  ...
  try {
    const detail = await updatePlatformInternalUserStatus(activeDetail.value.id, { enabled: nextEnabled })
    activeDetail.value = detail
    syncTableRow(detail)
    await fetchUserAudits()
...
<template>
  <div class="page-shell">
    ...
    <PageSectionCard title="筛选条件" description="按用户名、昵称和状态过滤平台内部账号。">
      <template #actions>
        <el-button v-if="access.canManagePlatformInternalUsers" type="primary" @click="createVisible = true">新建平台账号</el-button>
      </template>
    ...
    <PageSectionCard title="平台内部账号列表" description="仅展示平台内部账号，支持资料、角色、状态、密码和审计管理。">
    ...
    <UserDetailDrawer
      ...
      :can-edit="access.canManagePlatformInternalUsers"
      :can-assign-roles="access.canManagePlatformInternalUsers"
      :can-toggle-status="access.canManagePlatformInternalUsers"
      :can-reset-password="access.canManagePlatformInternalUsers"
